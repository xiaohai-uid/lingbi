...
class SyncService {
  ...
  /// 将项目元数据同步到 ZVec
  Future<void> syncProjectToDb(World project) async {
  ...
  /// 从磁盘扫描文档并更新 ZVec
  Future<List<Document>> syncFromDisk(
  ...
  /// 双向同步项目目录
  Future<List<Document>> fullSync(World project, String directoryPath) async {
    // 1. 从磁盘扫描所有 .md 文件
    final files = await _fileService.listDocuments(directoryPath);

    // 2. 从 ZVec 获取已知文档
    final knownDocs = await _zvecService
        .query('documents', filter: {'projectId': project.id});
