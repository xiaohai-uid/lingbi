...
class _GuidedWizardPageState extends State<GuidedWizardPage> {
  ...
  void _initApiKeyController() {
    final provider = ServiceLocator.instance.settingsService.selectedProvider;
    _apiKeyCtrl.text =
        ServiceLocator.instance.settingsService.getApiKey(provider);
  }
  ...
  void _onScreenTwoComplete() {
    ...
    if (title.isNotEmpty) {
      _machine.setDimension(
        ...
        WizardStepValue(selected: [title]),
      );
    }
    if (protagonist.isNotEmpty) {
      _machine.setDimension(
        WizardDimension.protagonist,
        WizardStepValue(selected: [protagonist]),
      );
    }
    if (worldview.isNotEmpty) {
      _machine.setDimension(
        WizardDimension.worldview,
        WizardStepValue(selected: [worldview]),
      );
    }
    if (goal.isNotEmpty) {
      _machine.setDimension(
    ...
    if (provider != 'free' && apiKey.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('请填写所选 Provider 的 API Key，或切换为免费模式'),
          duration: Duration(seconds: 2),
        ),
      );
      return;
    }
    if (provider != 'free' && apiKey.isNotEmpty) {
    ...
    locator.wizardCompletionWorkflow.execute(_machine).then((result) async {
      ...
      settings.updateOnboardingState(
        settings.onboardingState.copyWith(
          ...
          lastStep: 1,
          selectedProviderId: settings.selectedProvider,
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          Text('10 秒完成作品骨架', style: TextStyle(color: c.muted)),
          const SizedBox(height: 32),
          // 题材
          ...
          WizardCardSelector(
            ...
            multiSelect: false,
            initialSelected: _machine
                    .state.dimensionData[WizardDimension.platform]?.selected ??
                [],
            onChanged: (selected, custom) => _machine.setDimension(
  ...
  Widget _buildModelConfig(LingBiColors c) {
    final settings = ServiceLocator.instance.settingsService;
    final provider = settings.selectedProvider;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        _buildSectionTitle(c, '模型配置', '免费模式零配置，或选择 Provider 并填写 API Key'),
        const SizedBox(height: 12),
        DropdownButtonFormField<String>(
          initialValue: _providerLabels.containsKey(provider) ? provider : null,
          ...
          onChanged: (value) {
            ...
            _apiKeyCtrl.text = settings.getApiKey(value);
            setState(() {});
          },
          decoration: InputDecoration(
            labelText: '默认供应商',
        ...
        Row(
          children: [
            Text(
              '当前模型',
              style: TextStyle(fontSize: 13, color: c.fgSecondary),
            ),
            const Spacer(),
            const ModelSelector(),
          ],
