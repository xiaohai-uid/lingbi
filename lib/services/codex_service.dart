...
@Deprecated(
    'Use ICanonService instead. CodexService is replaced by CanonService in v4.0')
class CodexService implements ICodexService {
  @Deprecated('Use ICanonService instead')
  CodexService({required ZVecService zvecService}) : _zvec = zvecService;
  ...
  @override
  Future<CodexEntry> create(CodexEntry entry, {String? providerName}) async {
    await _zvec.upsert(_collectionName(entry.type), entry.id, entry.toJson());
    if (providerName != null) {
      try {
        final client = LLMFactory.create(providerName);
        if (client is FreeProvider) {
          final text = _embedText(entry);
          final embedding = await client.embed(text);
  ...
  Future<CodexEntry?> get(String id, CodexEntryType type) async {
    final result =
        await _zvec.get<Map<String, dynamic>>(_collectionName(type), id);
  ...
  @override
  Future<CodexEntry> update(CodexEntry entry, {String? providerName}) async {
  ...
  Future<List<CodexEntry>> semanticSearch(
    ...
    try {
      final client = LLMFactory.create(providerName);
      if (client is! FreeProvider) return [];
      final queryVector = await client.embed(query);
      final results = <CodexEntry>[];
