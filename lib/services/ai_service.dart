...
class AIService implements IAIService {
  ...
  void configureApiKey(String provider, String key) {
    ...
    if (idx >= 0) {
      _endpoints[idx] =
          _endpoints[idx].copyWith(apiKey: key.isEmpty ? null : key);
      ...
      final protocol =
          (provider == 'claude') ? Protocol.anthropic : Protocol.openai;
  ...
  List<ChatMessage> _buildMessages(String userMessage) {
    ...
    messages.add(ChatMessage(
      role: 'system',
      content: systemPromptFor(userMessage: userMessage),
    ));
  ...
  /// 构建带路由注入的系统提示词。
  ///
  /// 路由命中时追加 Available Skills，未命中时保持原系统提示词。
  ...
  String _baseSystemPrompt() {
    ...
    return '你是一个专业的 AI 写作助手，帮助用户进行小说创作。可以续写、改写、扩写文本，也可以分析结构和风格。';
  }

  String _routePrompt(RouteResult route) {
    return '''## Available Skills

- ${route.entry.displayName} (${route.entry.entryId})
- 路由得分：${route.score.toStringAsFixed(2)}
- 匹配证据：${route.matchedKeys.join(', ')}
''';
  }

  ModelInfo? get currentModelInfo {
    return ModelRegistry.instance.findModel(currentProvider.currentModelId,
  ...
  Stream<String> testGeneration(
      {String? providerId, int maxTokens = 100}) async* {
    final provider =
        providerId != null ? _resolveProvider(providerId) : currentProvider;
    final controller = StreamController<String>();
    _activeSubscription = provider.chat(messages: const [
      ChatMessage(role: 'user', content: '请用一句不超过 30 字的中文，描写雨夜中的旧车站。')
    ], maxTokens: maxTokens).listen(
      (chunk) => controller.add(chunk),
      onError: (Object error) {
        controller.addError(
            AiErrorMapper.map(error, provider: providerId ?? _currentProvider));
      },
  ...
  Future<ConnectionTestResult> testConnectionUnified(
    ...
    try {
      final resp = await provider.chatSync(
      ...
      return ConnectionTestResult(
          ...
          providerId: pid,
          message: '连接成功',
          responsePreview: _buildResponsePreview(t));
    } catch (e) {
      sw.stop();
  ...
  Stream<NormalizerEvent> normalizedChat(
      ...
      int maxTokens = 2048,
      bool treatAllAsCandidate = false}) async* {
    if (!_quota.tryConsume()) {
      yield const NormalizerError(message: '今日免费额度已用完。请配置自己的 API Key 或明天再试。');
      return;
    }
    yield* AiResponseNormalizer(treatAllAsCandidate: treatAllAsCandidate)
        .normalize(currentProvider.chat(
            messages: _buildMessages(message),
  ...
  Stream<String> chat(
    ...
    _activeSubscription = currentProvider
        ...
        .listen(
          ...
          onError: (Object error) {
            controller
                .addError(AiErrorMapper.map(error, provider: _currentProvider));
          },
          onDone: () => controller.close(),
        );
    yield* controller.stream;
  ...
  Future<String> analyzeStyle(String text) async {
    return currentProvider.chatSync(messages: [
      const ChatMessage(
          role: 'system',
          content:
              '你是一个文学风格分析专家。请分析以下文本的写作风格，包括：用词特点、句式结构、语气语调、修辞手法、节奏感。请用中文回复，输出结构化分析。'),
  ...
  Future<String> analyzeNovel(String text) async {
    return currentProvider.chatSync(messages: [
      const ChatMessage(
          role: 'system',
          content:
              '你是一个小说结构分析专家。请从以下文本中识别：角色、情节线、章节结构、叙事视角、主题、冲突类型。请用中文输出结构化报告。'),
      ChatMessage(role: 'user', content: text),
    ]);
  }
  ...
  Stream<String> continueWriting(String text) {
    return currentProvider.chat(messages: [
      const ChatMessage(
          role: 'system', content: '你是一个小说续写助手。请根据前文内容，自然地续写下一段。保持风格一致。'),
      ChatMessage(role: 'user', content: text),
    ], maxTokens: 1024);
  }
