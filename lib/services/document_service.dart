...
class DocumentService implements IDocumentService {
  ...
  Future<Document> createDocument({
    ...
    await _file.writeDocument(filePath, content.isEmpty ? '# $title\n\n' : content);
    final wordCount = _file.countWords(content.isEmpty ? '# $title\n\n' : content);
    final doc = Document(
      projectId: projectId,
      title: title,
      filePath: filePath,
  ...
  Future<List<Document>> getDocuments(String projectId) async {
    final results = await _zvec.query('documents', filter: {'projectId': projectId});
