...
class HookDensity {
  ...
  /// 执行爽点分析（规则引擎 + LLM 增强）
  Future<ReviewModuleResult> analyze(String text, {double targetDensity = 2.0}) async {
    ...
    final ruleDensity = wordCount > 0 ? (events.length / wordCount * 1000) : 0.0;

    if (ruleDensity < targetDensity * 0.5) {
      issues.add(Issue(
        type: 'low_hook_density',
        description: '爽点密度过低: ${ruleDensity.toStringAsFixed(1)}点/千字，目标 $targetDensity',
        severity: 7,
  ...
  /// 调用 LLM 进行爽点密度深度分析
  Future<Map<String, List<dynamic>>> _llmAnalyze(String text, List<HookEvent> existingEvents) async {
...
只输出 JSON，不要包含其他内容。''';
    ...
    if (jsonMatch == null) return {'events': <HookEvent>[], 'issues': <Issue>[]};
    ...
    final events = eventsJson.map((e) => HookEvent(
      type: e['type'] as String? ?? '打脸',
      description: e['description'] as String? ?? '',
      intensity: e['intensity'] as int? ?? 5,
    )).toList();

    final issues = issuesJson.map((i) => Issue(
