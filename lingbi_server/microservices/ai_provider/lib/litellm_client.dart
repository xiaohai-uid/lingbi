...
class RateLimitException extends LiteLLMException {
  ...
  const RateLimitException(String message, {this.retryAfter = const Duration(seconds: 30)})
      : super(message, statusCode: 429);
}

/// Exception thrown when a request times out.
class LiteLLMTimeoutException extends LiteLLMException {
  const LiteLLMTimeoutException(String message) : super(message, statusCode: null);
}
...
class LiteLLMClient {
  ...
  Future<Map<String, dynamic>> _executeWithRetry({
    ...
    for (int attempt = 0; attempt <= _maxRetries; attempt++) {
      try {
        ...
        throw LiteLLMException('Connection failed after $_maxRetries retries: ${e.message}');
      } on TimeoutException {
        if (attempt < _maxRetries) {
          await _waitBeforeRetry(attempt, null, 'timeout');
          continue;
        }
        throw LiteLLMTimeoutException('Request timed out after $_maxRetries retries');
      }
    }
  ...
  Future<void> _waitBeforeRetry(int attempt, int? statusCode, String errorBody) async {
  ...
  LiteLLMException _createException(int statusCode, String errorBody) {
    ...
    try {
      ...
      message = json['error']?['message'] ?? json['error']?.toString() ?? errorBody;
    } catch (_) {
      message = errorBody;
    }
    ...
    switch (statusCode) {
      ...
      case 400:
        return LiteLLMException('Bad request: $message', statusCode: statusCode, body: errorBody);
      case 401:
        return LiteLLMException('Authentication failed: $message', statusCode: statusCode, body: errorBody);
      case 404:
        return LiteLLMException('Not found: $message', statusCode: statusCode, body: errorBody);
      case 500:
        return LiteLLMException('Internal server error: $message', statusCode: statusCode, body: errorBody);
      default:
        return LiteLLMException('HTTP $statusCode: $message', statusCode: statusCode, body: errorBody);
    }
  }
