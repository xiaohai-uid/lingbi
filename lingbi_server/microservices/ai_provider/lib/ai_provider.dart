...
class AIProviderMicroservice {
  ...
  Future<void> _handleChat(HttpRequest request, ChatRequest chatRequest) async {
    ...
    try {
      ...
      request.response.write('data: ${json.encode({'error': e.toString()})}\n\n');
      await request.response.flush();
    }
  ...
  Future<void> _handleStyleAnalyze(HttpRequest request, ChatRequest chatRequest) async {
    request.response.headers.contentType = ContentType('text', 'event-stream');
    request.response.headers.set('Cache-Control', 'no-cache');
    request.response.headers.set('Connection', 'keep-alive');
  ...
  Future<void> _handleNovelAnalyze(HttpRequest request, ChatRequest chatRequest) async {
    request.response.headers.contentType = ContentType('text', 'event-stream');
    request.response.headers.set('Cache-Control', 'no-cache');
    request.response.headers.set('Connection', 'keep-alive');
  ...
  Future<void> _handleContinueWriting(HttpRequest request, ChatRequest chatRequest) async {
    request.response.headers.contentType = ContentType('text', 'event-stream');
    request.response.headers.set('Cache-Control', 'no-cache');
    request.response.headers.set('Connection', 'keep-alive');

    try {
      await for (final chunk in chatService.continueWritingStream(chatRequest)) {
        request.response.write('data: ${json.encode(chunk)}\n\n');
        await request.response.flush();
      }
      request.response.write('data: [DONE]\n\n');
  ...
  Future<void> _handleEmbedding(HttpRequest request, Map<String, dynamic> body) async {
    try {
  ...
  Future<void> _handleAddModel(HttpRequest request, Map<String, dynamic> body) async {
    try {
  ...
  Future<void> _handleSetActiveModel(HttpRequest request, String modelId) async {
    try {
      await chatService.setActiveModel(modelId);
      request.response.headers.contentType = ContentType.json;
      request.response.write(json.encode({'success': true, 'activeModel': modelId}));
      await request.response.close();
    } catch (e) {
      request.response.statusCode = HttpStatus.internalServerError;
      request.response.write(json.encode({'error': e.toString()}));
