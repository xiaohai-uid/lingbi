...
class DatabaseService {
  ...
  List<Document> list({String? projectId}) {
    ...
    if (projectId != null) {
      ...
      rows =
          _db.select('SELECT * FROM documents ORDER BY updated_at DESC').toList();
    }
  ...
  Document? update(String id, {String? title, String? content, String? projectId}) {
  ...
  OutlineResult getOutline(String documentId) {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (headingMatch != null) {
        ...
        entries.add(OutlineEntry(level: level, title: title, lineNumber: i + 1));
      }
    }
  ...
  DocumentStats getStats(String documentId) {
    ...
    final characterCount = content.replaceAll('\n', '').replaceAll('\r', '').length;
